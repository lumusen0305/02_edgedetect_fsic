...
namespace EdgeDetect_IP 
{
  ...
  const int maxImageHeight = 360; 
  
  const int kernel[3] = {1, 0, -1};
    
  // Define some bit-accurate types to use in this model
  typedef ac_int<8,false>                  pixelType;    // input pixel is 0-255
  typedef ac_int<16,false>                 pixelType2x;  // two pixels packed
  typedef ac_int<32,false>                 pixelType4x;  // two pixels packed
  typedef ac_int<64,false>                 pixelType8x;  // two pixels packed
  ...
  // typedef hls::stream<int, 34> Stream_t;
  // typedef ac_int<34,false>  Stream_t;  // two pixels packed
    struct Stream_t {
        uint32 pix; // Choose appropriate types based on your definitions
        bool sof;
        bool eol;
    };

  // typedef StreamStruct Stream_t; // This makes Stream_t an alias for StreamStruct
